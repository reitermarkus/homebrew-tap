cask "pam-touch-id" do
  ...
  postflight do
    ...
    system_command "/usr/sbin/chown", args: ["root:wheel", "/usr/local/lib/pam/pam_touchid.so.2"],
                                      sudo: true
  ...
  uninstall_preflight do
    system_command "/usr/bin/sed", args: ["-i", "", "-E", "/auth +sufficient +pam_touchid.so/d", "/etc/pam.d/sudo"],
                                   sudo: true
    system_command "/usr/sbin/chown", args: ["root:staff", "/usr/local/lib/pam/pam_touchid.so.2"],
                                      sudo: true
